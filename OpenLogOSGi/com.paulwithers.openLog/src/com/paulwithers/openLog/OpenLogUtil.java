...
public class OpenLogUtil {
	...
	/**
	 * Logs an error / throwable for a specific Session at a specific severity
	...
	/**
	 * Logs an event for a specific Session at a specific severity for a
	 * specific document
	 *
	 * @param s
	...
	static void debugPrint(Throwable ee) {
		...
		try {
			...
			final int debugLevel = Integer.parseInt(OpenLogItem.olDebugLevel);
			if (debugLevel >= 1) {
	...
	static String getIniVar(String propertyName, String defaultValue) {
		try {
			final String newVal = ExtLibUtil.getCurrentSession().getEnvironmentString(propertyName, true);
